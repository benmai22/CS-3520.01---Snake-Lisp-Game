...
def generate_gene(champs, mutate_rate):
...
def run_ga():
    ...
    if input("Use default values [y/n]: ").startswith("n"):
        ...
        epochs = int(input("Enter number of generations to run for: "))
    ...
    for epoch in range(0, epochs):
        ...
        for i in range(0, game_count):
            snake = Snake(game_size, dot_size, delay, distance_penalty, food_reward)
            if i < number_champs:  # Make sure the champions genes transfer to the next generation
                gene = champs[i][2]
            else:
                gene = generate_gene(champs, mutation_rate)
...
if input("Run snake manually? (y/n) ").lower()[:1] == 'y':
    run_manual()
else:
    run_ga()